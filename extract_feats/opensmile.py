...
def load_feature(config, train: bool) -> Union[Tuple[np.ndarray], np.ndarray]:
    ...
    Returns:
        ...
        - X (np.ndarray): 预测特征
    """
    # /home/wangshiyao/wangshiyao_space/exp3/Speech-Emotion-Recognition-master/features/7-category/train_opensmile_emodb.csv
    # feature_path = os.path.join(config.feature_folder, "train.csv" if train == True else "predict.csv")
    feature_path = os.path.join(config.feature_folder, "train_opensmile_emodb.csv" if train == True else "predict.csv") # wsy 
...
def get_data(config, data_path: str, train: bool) -> Union[Tuple[np.ndarray], np.ndarray]:
    ...
    # 特征存储路径
    feature_path = os.path.join(config.feature_folder, "train_opensmile_emodb.csv" if train == True else "predict.csv")
